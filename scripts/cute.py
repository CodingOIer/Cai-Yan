...
def setup():
    ...
    try:
        with open('./prompt/cute.md', 'r', encoding='utf-8') as f:
            SYSTEM_PROMPT = f.read()
    except Exception as e:
        log.error(f'获取系统提示词失败 {e}')


def getCuteResponses(driver):
    responses = []
    try:
        ...
        log.error(f"提取对方回复时出错: {e}")
    return responses


def getMeInputs(driver):
    inputs = []
    try:
        seleniumUi.WebDriverWait(driver, 5).until(
            seleniumEc.presence_of_element_located(
...
def main():
    ...
    try:
        ...
        attempts = 0
        time.sleep(0.3)
        initialCuteResponses = getCuteResponses(driver)
        if initialCuteResponses:
            lastInitialMessage = initialCuteResponses[-1]
            log.info(f"初始对方消息: {lastInitialMessage}")
            conversationHistory.append(
                {"role": "assistant", "content": lastInitialMessage}
            )
        else:
            log.error("未能捕获到初始对方消息。")
        while attempts < MAX_ATTEMPTS:
            attempts += 1
            log.info(f"第 {attempts} 次尝试")
            if not conversationHistory and not initialCuteResponses:
            ...
            CuteResponses = getCuteResponses(driver)
            myInputs = getMeInputs(driver)
            newCuteResponse = ""
            if CuteResponses:
                newCuteResponse = CuteResponses[-1]
                log.info(f"对方回复: {newCuteResponse}")
            else:
                log.warning("未能获取到新的对方回复。")
            if myInputs:
            ...
            if newCuteResponse:
                if (
                    not conversationHistory
                    or conversationHistory[-1]["content"] != newCuteResponse
                    or conversationHistory[-1]["role"] != "assistant"
                ):
                    conversationHistory.append(
                        {"role": "assistant", "content": newCuteResponse}
                    )
            else:
                log.warning("对方没有新的回复，即使等待后也是如此。")
            try:
                ...
                if successMessageElements:
                    ...
                    if (
                        ...
                        and "恭喜你" in newCuteResponse
                    ):
                        finalGuess = conversationHistory[-1]['content']
                    log.success(f"最终猜测的角色是: {finalGuess}")
                    break
            except Exception:
                pass
